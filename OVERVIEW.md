...
The Opalescent protocol spec defines a way to create and participate in shared, mutable states with other peers.
...
Do not try to build a cryptocurrency with Opalescent (unless you really, really want to).
There are some things you cannot do with Opalescent and similar peer-to-peer databases.
...
With Opalescent we allow concurrent and async updates to occur and be merged.
...
Designing applications with Opalescent can feel very different.
Understanding it's differences and a few tricks will result in a better design.
One of the biggest differences is that there is no idea of a global and consistent state in Opalescent, peers replicas are allowed to diverge and converge over and over.
